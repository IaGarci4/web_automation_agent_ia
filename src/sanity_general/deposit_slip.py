...
async def tomar_foto(flow) -> bool:
    ...
    if not await _click(tomar, force=True):
        logger.warning("[Ficha] No se pudo pulsar 'Take Picture'.")
        return False
    await page.wait_for_timeout(1_200)
    guardar = page.locator("button#id-button:visible").filter(has_text=RE_GUARDAR)
    if not await _click(guardar, force=True):
        # Respaldo: algunos temas no usan #id-button
        guardar = page.locator("button:visible").filter(has_text=RE_GUARDAR)
        if not await _click(guardar, force=True):
            logger.warning("[Ficha] No se pudo pulsar 'Save Picture'.")
            return False
