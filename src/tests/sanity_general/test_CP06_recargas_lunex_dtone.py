"""
CP06 — Recargas (Top Ups) · Lunex y DTOne.
(Rescate de `HERMES2-qa/.../test_CP06_Recargas_Lunex_DTOne.py`.)
...
COMO CORRER:
    pytest src/tests/sanity_general -k CP06 -v -s --headed
"""
import os
import pytest
...
logger = get_logger("CP06")

# ── Datos del caso (override por env; cambian por ambiente si aplica) ─────────
MEGA_PHONE   = os.getenv("CP06_MEGA_PHONE",   "3322313245")
MEGA_COUNTRY = os.getenv("CP06_MEGA_COUNTRY", "Mexico")
MEGA_AMOUNT  = os.getenv("CP06_MEGA_AMOUNT",  "$2")
MEGA_NAME    = os.getenv("CP06_MEGA_NAME",    "TEST")

REG_BENEF_PHONE = os.getenv("CP06_BENEF_PHONE", "3322313245")
REG_BENEF_NAME  = os.getenv("CP06_BENEF_NAME",  "TEST RECARGA")
REG_CUST_PHONE  = os.getenv("CP06_CUST_PHONE",  "7774533317")
REG_CUST_NAME   = os.getenv("CP06_CUST_NAME",   "TEST")
REG_CUST_LAST   = os.getenv("CP06_CUST_LAST",   "TEST")

# Sub-flujos activables por env (por si se quiere correr solo uno en la demo).
HACER_MEGA    = os.getenv("CP06_MEGA",    "1").strip().lower() in ("1", "true", "si", "yes")
HACER_REGULAR = os.getenv("CP06_REGULAR", "1").strip().lower() in ("1", "true", "si", "yes")

EVIDENCE = "CP06_recargas_lunex_dtone"


@pytest.mark.sanity_general
@pytest.mark.asyncio
@pytest.mark.parametrize("language,width,height", [("English", 1366, 768)])
async def test_CP06_recargas_lunex_dtone(logged_page: Page, language, width, height):
    ...
    if HACER_MEGA:
        logger.info("[CP06] MEGA TOP UPS · abriendo…")
        ...
        logger.info("[CP06] MEGA TOP UPS listo (Send visible=%s).", mega_ok)

    # ── Steps 4–8: TOP UPS REGULAR ───────────────────────────────────────────
    if HACER_REGULAR:
        logger.info("[CP06] TOP UPS REGULAR · abriendo…")
        ...
        logger.info("[CP06] TOP UPS REGULAR listo (Send visible=%s).", regular_ok)
    ...
    assert mega_ok and regular_ok, (
        "CP06: alguno de los formularios de recarga no llegó al botón de envío "
        "visible (revisar selectores del iframe Lunex / campos en test-hermes).")
    logger.info("[CP06] Recargas Lunex y DTOne — formularios validados (sin enviar) ✓")
