...
async def _esperar_modal(page, timeout_ms: int = 2_500) -> bool:
    """Espera un poco a que el aviso APAREZCA. True si salió.

    Sin esta espera, `_cerrar_warning` preguntaba «¿hay modal?» en el mismo
    instante del clic, cuando Angular todavía no lo había pintado. Concluía
    que no había nada que cerrar y seguía — con el aviso apareciendo medio
    segundo después y bloqueando la navegación. El caso terminaba buscando
    los controles del reporte mientras la app seguía en /transfers.
    """
    espera = 0
...
async def _cerrar_warning(page, timeout_ms: int = 3_000, vueltas: int = 3,
                          esperar_aparicion: bool = False) -> bool:
    ...
    `esperar_aparicion`: úsalo justo después de un clic que PUEDE abrirlo. Sin
    eso se comprueba el instante equivocado (ver `_esperar_modal`).
    """
    if esperar_aparicion and not await _esperar_modal(page):
        return False
    cerrado = False
...
async def _abrir_reporte(page, dropdown_testid: str, desc: str,
                         testid_confirma: str = None) -> bool:
    """Reportes → <submenú>, CONFIRMANDO que la pantalla cambió.

    ## Por qué se confirma

    Esta función decía «abierto» y devolvía `True` en cuanto los dos clics no
    lanzaban excepción. Con el aviso de salida de por medio, los clics
    ocurrían, la navegación NO, y la app se quedaba en `/transfers`. El caso
    seguía adelante buscando `reports-by-cashier-search-button` en una
    pantalla que no lo tiene: dos timeouts de 15 s y un fallo final —«no se
    pudo disparar la impresión»— que señalaba al botón de imprimir cuando el
    problema estaba tres pasos antes.

    Confirmar cuesta nada y convierte 78 s de despiste en ~20 s con el dedo
    ...
    try:
        ...
        await page.get_by_test_id(dropdown_testid).first.click(force=True,
                                                               timeout=TIMEOUT)
        # El aviso llega DESPUÉS del clic: hay que esperarlo, no mirarlo de reojo.
        await _cerrar_warning(page, esperar_aparicion=True)
    ...
    if testid_confirma:
        try:
            await page.get_by_test_id(testid_confirma).first.wait_for(
                state="visible", timeout=15_000)
            logger.info("[Balance] %s abierto (confirmado por '%s').",
                        desc, testid_confirma)
            return True
        except Exception:
            pass
    else:
        ...
        while espera <= 15_000:
            if page.url != url_antes:
                logger.info("[Balance] %s abierto (URL: %s).", desc, page.url)
                return True
            await page.wait_for_timeout(300)
            espera += 300

    logger.warning(
        "[Balance] NO se llegó a %s: la pantalla no cambió y la app sigue en "
        "%s. Lo más probable es el aviso «¿salir de esta página?»: si quedó "
        "abierto, su overlay se come los clics. Mira la captura del caso.",
        desc, page.url)
    return False
...
async def abrir_balance_por_cajero(page) -> bool:
    ...
    return await _abrir_reporte(page, DROPDOWN_CAJERO,
                                "Reportes > Balance por Cajero",
                                testid_confirma=CAJERO_BTN_BUSCAR)
