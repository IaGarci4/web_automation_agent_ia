...
async def _click_boton_add_info_visible(flow) -> bool:
    ...
    for sel in selectores:
        ...
        for i in range(n):
            b = loc.nth(i)
            try:
                # Visible Y habilitado. Y con timeout corto: hay varios
                # `btn-accept` en el DOM (uno por pestaña Envío 1/2/3) y si el
                # candidato no sirve conviene descartarlo en 1,5 s, no en 4 —
                # se recorren varios antes de dar con el del modal.
                if not await b.is_visible() or not await b.is_enabled():
                    continue
                await b.click(timeout=1_500)
...
async def abrir_info_adicional(flow, tipo: str = "cash", timeout: int = None) -> bool:
    ...
    ⏱️ POR QUÉ ANTES TARDABA ~6 s DE MÁS. La comprobación previa era
    `esperar_ficha(timeout=2_000)`, y esa función, al fallar, ENCADENA otra
    espera de 4 s sobre el input de país. Total: 6 s garantizados antes de
    mirar siquiera el botón — y se pagaban SIEMPRE, porque cuando se llama a
    esta función la ficha todavía no está abierta (es lo que se viene a hacer).

    Ahora la comprobación previa es instantánea (`_ficha_visible`, un
    `is_visible` sin espera) y el botón se sondea cada 200 ms. El modal sigue
    pudiendo tardar mucho en aparecer en algunos flujos, así que tras los
    primeros 6 s el sondeo se relaja a 1 s: rápido en el caso normal, paciente
    en el raro."""
    if timeout is None:
        timeout = MODAL_TIMEOUT_MS
    # Instantáneo: si la ficha ya está abierta, no hay nada que hacer.
    ...
    while transcurrido < timeout:
        ...
        if await _click_boton_add_info_visible(flow):
            if await esperar_ficha(flow, timeout=15_000):
                ...
                return True
        # Sondeo fino al principio (el modal casi siempre ya está en pantalla),
        # y más espaciado después para no quemar CPU en la espera larga.
        paso = 200 if transcurrido < 6_000 else 1_000
        await flow.page.wait_for_timeout(paso)
        transcurrido += paso
        if transcurrido % 10_000 < paso:
