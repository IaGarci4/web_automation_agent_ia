...
async def seleccionar_impresora(page, impresora: str = None) -> bool:
    ...
    try:
        ...
        # ── ¿Se abrió el panel, o es que no hay impresoras? ─────────────────
        # «Disponibles: (ninguna)» decía las dos cosas a la vez, y son
        # diagnósticos opuestos: uno manda a revisar el emulador y el otro,
        # el selector. Es el mismo error que costó una tarde con el Reason de
        # Chronos. Se comprueba primero que el PANEL exista.
        panel = page.locator(".p-dropdown-panel, .p-dropdown-items-wrapper")
        try:
            await panel.first.wait_for(state="visible", timeout=6_000)
            abierto = True
        except Exception:
            abierto = False
        ...
        try:
            ...
            if not abierto:
                logger.warning(
                    "[MO] El desplegable de impresora NO llegó a abrirse (no "
                    "hay panel en el DOM). Esto NO dice nada sobre el "
                    "emulador: es el clic sobre el combo lo que no prendió.")
            elif not textos:
                logger.warning(
                    "[MO] El desplegable ABRIÓ y está VACÍO: Hermes no recibió "
                    "ninguna impresora del Hardware Agent. El emulador '%s' no "
                    "está llegando a Windows. Diagnostícalo con:\n"
                    "    python tools/kra1527_diagnostico_perifericos.py",
                    impresora)
            else:
                logger.warning(
                    "[MO] '%s' no está entre las %d impresora(s) que sí "
                    "reporta el agente: %s", impresora, len(textos),
